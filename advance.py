...
def evaluate_advance(model, dev=None):
    ...
    data_root = Path(__file__).parent / 'datasets'/'AdvancedData'

    images = list((data_root / 'ADVANCE_vision' / 'vision').glob('*/*.jpg'))
    images.sort()
...
if __name__ == '__main__':
    ...
    # args = parser.parse_args(["./models/TL-RN50"])
    path_models = Path("./models/TL-RN50")
    cfg.merge_from_file(path_models / 'config.yml')
    cfg.freeze()
    print(cfg)
    ...
    model.load_state_dict(torch.load(path_models / 'checkpoints/best.pt', map_location=dev))
